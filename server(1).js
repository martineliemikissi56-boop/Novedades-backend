...
// Ajoute la colonne avatar_url si la base existait déjà avant cette mise à jour
try {
...
app.post("/api/groups/:groupId/messages", authMiddleware, (req, res) => {
  ...
  const info = db
    .prepare("INSERT INTO group_messages (group_id, sender_id, text) VALUES (?, ?, ?)")
    .run(groupId, req.user.id, text.trim());
...
app.post("/api/messages", authMiddleware, (req, res) => {
  ...
  const info = db
    ...
    .run(req.user.id, receiverId, text.trim());
